...
public class PvPManager {
        ...
        if (!map.isEmpty()) {
            ...
            for (Player player : playersInPvp) {
                ...
                if (timeRemaining <= 0 || (settings.isDisablePvpInIgnoredRegion() && isInIgnoredRegion(player))) {
                    ...
                    if (!bypassed) {
                        bossbarManager.updateBossBar(player, timeRemaining);
                    }
                    if (bypassed) {
                        silentPvpMap.put(player, timeRemaining);
                    } else {
                        pvpMap.put(player, timeRemaining);
                    }
    ...
    public Map<Player, Integer> getPvpMap() {
        return pvpMap;
    }

    public Set<Player> getOpponents(Player player) {
        return pvpOpponents.getOrDefault(player, new HashSet<>());
    }
    ...
    private void addOpponent(Player player, Player opponent) {
        pvpOpponents.computeIfAbsent(player, k -> new HashSet<>()).add(opponent);
    }
    ...
    public void startPvpManually(Player player, int seconds) {
        ...
        if (bypassed) {
            silentPvpMap.put(player, seconds);
        } else {
            pvpMap.put(player, seconds);
            bossbarManager.setBossBar(player, seconds);
        ...
        try {
            Bukkit.getPluginManager().callEvent(new PvpStartedEvent(player, null, seconds, PvPStatus.ALL_NOT_IN_PVP));
        } catch (Exception e) {
            plugin.getLogger().warning("Ошибка при вызове PvpStartedEvent: " + e.getMessage());
        }
    }
    ...
    public void stopPvP(Player player) {
        for (Map.Entry<Player, Set<Player>> entry : pvpOpponents.entrySet()) {
